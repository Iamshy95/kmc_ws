# 🛠️ 현장 실전 대응 가이드

### 0. 깃 클론
레포지토리 이름이 `kmc_ws`이므로 홈 디렉토리에서 바로 클론하면 경로가 자동으로 맞춰집니다.

```bash
cd ~
git clone https://github.com/Iamshy95/kmc_ws.git
```

### 1. 하드웨어 연결 및 포트 확인
차량 USB를 연결한 후, 포트 권한을 부여합니다.

```bash
# 1. 포트 확인 (보통 /dev/ttyUSB0 확인)
ls -l /dev/ttyUSB*

# 2. 포트 권한 부여 (읽기/쓰기 권한 허용)
# 비밀번호를 물어보면 입력하세요.
sudo chmod 666 /dev/ttyUSB0
```

### 2. 통합 빌드 (SDK + ROS2)
워크스페이스 최상위(`~/kmc_ws`)에서 스크립트를 실행하면 SDK와 ROS 패키지가 순서대로 빌드됩니다.

```bash
cd ~/kmc_ws
./src/controller/scripts/build.sh
```

### 3. 터미널 환경 설정
**모든 새 터미널**을 열 때마다 반드시 실행해야 합니다.

```bash
cd ~/kmc_ws
source src/controller/scripts/env.sh
```

### 4. 노드 실행 순서 (총 3개 터미널)

#### 터미널 1: KMC 드라이버 (C++)
* **포트/ID 변경:** 명령어의 옵션만 수정하면 즉시 반영됩니다. (재빌드 불필요)
* 아래 명령어의 `car_id:=1`와 `port:=...`를 현장 상황에 맞게 변경하세요.

```bash
ros2 run kmc_hardware kmc_driver_node --ros-args -p port:=/dev/ttyUSB0 -p car_id:=1
```

#### 터미널 2: RSU 인프라 (Python)
* **주의:** 실행 전 코드를 열어 `self.id_map`과 `self.hv_ids`가 현장 차량과 일치하는지 확인하세요.
* `nano` 에디터 사용법: 수정 후 `Ctrl+O` (저장) -> `Enter` -> `Ctrl+X` (종료)

```bash
# 실행
python3 src/controller/controller/rsu.py
```

#### 터미널 3: 주행 제어 (Python)
* **필수 수정:** 실행 전 코드를 열어 `self.car_id = 1`을 **현재 차량 번호**로 수정해야 합니다.
* **경로 확인:** `path_file` 경로가 `path1.csv`인지 `path2.csv`인지 확인하세요.

```bash
# 코드 수정 (Car ID 및 경로 파일 확인)
nano src/controller/controller/drive_basic.py
```

```bash
# 실행
python3 src/controller/controller/drive_basic.py
```

---

### 🚨 비상 대응 체크리스트

1.  **경로 에러 (`FileNotFoundError`)**
    * 현재 폴더가 `~/kmc_ws`인지 확인하세요 (`pwd` 명령어 사용).
    * 코드 내의 파일 경로(`path1.csv` 등)가 실제 파일명과 대소문자까지 일치하는지 확인하세요.

2.  **통신 에러 (Open Error / Permission Denied)**
    * 드라이버 노드가 실행되지 않으면 `ls -l /dev/ttyUSB*`로 포트가 연결되어 있는지 확인하세요.
    * `sudo chmod 666 /dev/ttyUSB0` 명령어를 다시 실행하세요.

3.  **종료 순서 (안전 제일)**
    * 1순위: **[주행 노드(Py)]** 종료 (`Ctrl+C`)
    * 2순위: 차량이 완전히 멈췄는지 눈으로 확인
    * 3순위: **[드라이버 노드(C++)]** 종료