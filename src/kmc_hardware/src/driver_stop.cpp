...
public:
  KmcHardwareDriverDemoNode() : Node("kmc_hardware_driver_demo_node") {
    ...
    // [수정] 노드 시작 즉시 속도 0 명령 전송 및 초기 시간 기록
    last_cmd_v_ = 0.0f;
    last_cmd_w_ = 0.0f;
    ...
    if (command_refresh_hz_ > 0.0) {
      ...
      cmd_timer_ = create_wall_timer(period, [this]() {
        ...
        // [수정] Watchdog: 마지막 명령 수신 후 command_timeout_ms가 지나면 속도를 0으로 리셋
        const double elapsed_seconds = (this->now() - last_cmd_received_.value()).seconds();
        if (elapsed_seconds > (command_timeout_ms_ / 1000.0)) {
          last_cmd_v_ = 0.0f;
          last_cmd_w_ = 0.0f;
        }
  ...
  ~KmcHardwareDriverDemoNode() override { 
    // [수정] 안전을 위해 종료 시 정지 명령 전송
    driver_.setCommand(0.0f, 0.0f);
    driver_.stop(); 
  }
